<template>
  <div class="abilityReady-content">
    <el-button type="primary" plain  @click="$router.push({path:'/abilityResult/add'})" class="addBtn">新增</el-button>
    <el-table
      :data="tableData"
      border
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button
            size="medium"
            type="primary"
            @click="delOpp(scope.$index,scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      getReadyList:function(){
        ...
        this.$axios
          .post("/feedback/getFeedbackList", obj)
          .then(res => {
            if(res.code="000000"){
      ...
      /*查看*/
      lookInfo(index,row){
        this.$router.push({path: '/abilityResult/show',query: {id:row.trainingFeedbackId}})
      },
      /*编辑*/
      editInfo(index,row) {
        this.$router.push({path: '/abilityResult/add',query: {id:row.trainingFeedbackId}})
      },
      //删除
      delOpp(index,val){
        this.$confirm('确认要删除此记录吗?', '删除确认', {
          ...
          console.log(obj)
          this.$axios
            .post("/feedback/deleteFeedback", obj)
            .then(res => {
              if(res.code=="000000"){
                this.$message({
                  type: 'success',
