    ...
    <div class="addParent">
      <commonTitle>
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item>编辑家长</el-breadcrumb-item>
        </el-breadcrumb>
      </commonTitle>
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="170px"  label-position="left" class="demo-ruleForm">
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
       ...
       tutorial: [
          {required:true, message: '请选择服务导师', trigger: 'change' }
        ],
       /* */
      },
      isEdit:false,// 是否是编辑
  ...
  methods: {
    ...
    getUserInfo(){
      ...
      var $this=this;
      this.$axios
        .post("/user/getParent", {"userId":$this.id})
        .then(res => {
          if(res.code=="000000"){
            var obj=res.result;
            $this.ruleForm=res.result;
            $this.tutorialList.forEach(val=>{
              if(val.id==obj.tutorialCode){
               $this.$set($this.ruleForm,"tutorial",val)
              }
            })
            console.log($this.ruleForm.tutorial,"===")
            $this.ruleForm.hopeChildSubject=JSON.parse(obj.hopeChildSubject);
          }else{
